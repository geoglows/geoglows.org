---
title: Resources & Training | GEOGLOWS
blocks:
  - type: richText
    eyebrow: Resources & training
    heading: Learn to work with GEOGLOWS
    body: >-
      Everything you need to start using GEOGLOWS streamflow forecasts, from
      first-time tutorials to the open data services and developer tools. New to
      the initiative? Begin with the getting started guide, then explore the
      resources below.
  - type: richText
    heading: Guides & tutorials
    background: muted
    body: >-
      - **[Getting started guide](/getting-started)** — the fastest path from zero to your first forecast

      - **[Video tutorials & demos](/videos)** — walkthroughs of the Hydroviewer, data services, and applications

      - **[The GEOGLOWS Service](/service)** — how the model, forecasts, and retrospective fit together
  - type: richText
    heading: Data & developer tools
    body: >-
      - **[Tools & data](/tools)** — the full GEOGLOWS toolset and data services

      - **[Open Hydroviewer](https://apps.geoglows.org)** — explore forecasts for any river reach

      - **[GEOGLOWS on GitHub](https://github.com/geoglows)** — the Python package, model code, and open-source repositories
  - type: richText
    heading: Science & community
    background: muted
    body: >-
      - **[Publications library](/publications)** — peer-reviewed research using GEOGLOWS

      - **[News & events](/news)** — releases, training announcements, and community updates

      - **[Meet the community](/about)** — the working groups, partners, and people behind GEOGLOWS
  - type: closingCta
    heading: Need help getting started?
    ctas:
      - { label: Getting started guide, href: /getting-started, style: amber }
      - { label: Contact the team, href: "mailto:secretariat@geoglows.org", style: navy }
---
